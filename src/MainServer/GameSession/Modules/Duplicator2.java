...
public class Duplicator2 implements Runnable {
    Space shared, out, conns;
    String spaceKeyIn, spaceKeyOut;
    // T is 1/F, where F is update frequency
    int T;

    public Duplicator2(Space shared, Space out, Space conns, String spaceKeyIn, String spaceKeyOut) {
        this.shared = shared;
        this.out = out;
        this.conns = conns;
     ...
     * the changelist Duplicator2 and slower (higher) for the sync Duplicator2
...
//        System.out.println("Duplicator2@"+ Thread.currentThread().getId() + " finished");
    }
}
